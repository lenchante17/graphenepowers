...
This is the internal classification module used by `graphenepowers:using-graphenepowers`.
...
**Core principle:** A single high-risk dimension can force a heavier route even when the total score looks small.
...
- `coupling = 3` -> at least `Small Task`
- `external dependency = 3` -> at least `Small Task` and human gate
- `risk / cost = 3` -> at least `Small Task` and human gate
...
- `Micro` -> `routing/micro.md`
- `Small Task` -> `routing/small-task.md`
...
| Reclassification raises grade | Human gate |
